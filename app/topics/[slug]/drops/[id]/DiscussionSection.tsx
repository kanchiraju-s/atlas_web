        ...
        {replying && (
          <div style={{ marginTop: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              >{replyMut.isPending ? 'Posting…' : 'Reply'}</button>
            </div>
          </div>
        )}
      </div>
...
export function DiscussionSection({ dropId }: { dropId: string }) {
  const user = useAuthStore((s) => s.user);
  const qc = useQueryClient();
  const [comment, setComment] = useState('');

  const discussions = useQuery({ queryKey: ['discussions', dropId], queryFn: () => getDiscussions(dropId) });

  const createMut = useMutation({
    mutationFn: (content: string) => createDiscussion(dropId, content),
    onSuccess: () => { setComment(''); qc.invalidateQueries({ queryKey: ['discussions', dropId] }); },
  });
  ...
  return (
    <div>
      ...
      {user ? (
        <div style={{ marginBottom: 32 }}>
          <textarea
            ...
            placeholder="Add to the discussion…" rows={3}
            style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', fontSize: 14, color: 'var(--text-primary)', outline: 'none', resize: 'none', marginBottom: 10, transition: 'border-color 150ms' }}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            ...
            >{createMut.isPending ? 'Posting…' : 'Post'}</button>
          </div>
        </div>
      ) : (
      ...
      {discussions.isLoading ? (
        ...
        <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>No discussion yet.</p>
      ) : (
        <div>{discussions.data?.map((d, idx) => (
          <div key={d.id} style={{ borderTop: idx > 0 ? '1px solid var(--border)' : 'none' }}>
            <Thread disc={d} dropId={dropId} />
          </div>
        ))}</div>
      )}
    </div>
